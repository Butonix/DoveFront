<template>
	<div>
		...
		<v-data-table :options.sync="options"
			...
			:loading="loading"
			loading-text="Loading municipality wards..."
			class="elevation-1"
			:footer-props="{
			...
			<template #top>
				...
				>
					...
					>
						...
						<span v-if="$vuetify.breakpoint.smAndUp">Municipality Ward</span>
					</v-btn>


					<add-municipality-ward-form-dialog @reload="initialize" />
				</v-toolbar>
			</template>
				...
				>
					...
					<template #input>
						<v-text-field
							...
							label="Municipality Ward Number"
							single-line
							counter
							type="number"
						/>
		...
		<admin-delete-item-dialog
			model-name="municipality_ward"
			delete-action="location/deleteMunicipalityWard"
			@reload="initialize"
		/>
	</div>
</template>
...
export default {
	name: "MunicipalityWardsTable",
	components: {
		AddMunicipalityWardFormDialog: () => import("@/views/location/AddMunicipalityWardFormDialog")
	},
	mixins: [
		AdminTableList,
	...
	methods: {
		openCreateFormDialog() {
			this.$bus.emit("open-add-municipality-ward-form")
		},
		async initialize(val = null) {
			this.loading = true
			if (!val) val = this.options.page
			await this.$store.dispatch("location/filterMunicipalityWard", {page: val})
			this.items = this.municipalityWards
			this.totalItems = this.municipalityWards.count
			this.loading = false
		},
		async updateName(item) {
			try {
				await this.$api.patch(util.format(urls.location.municipalityWardDetail, item.id), {name: this.nameToUpdate})
				await this.openSnack("Municipality ward updated successfully", "success")
			} catch (e) {
				const status = parseInt(e.response.status.toString())
				if (status === 400) {
					await this.openSnack(e.response.data["name"][0])
				} else {
					await this.openSnack("Municipality ward update failed")
				}
			}
		...
		async updateNumber(item) {
			try {
				await this.$api.patch(util.format(urls.location.municipalityWardDetail, item.id), {number: this.numberToUpdate})
				await this.openSnack("Municipality ward updated successfully", "success")
